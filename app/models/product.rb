    ...
    if type == 'jan' then
      ...
      query.each do |squery|
        ...
        if asin != nil then
          ...
          if jj.key?(squery) == false then
            converter_list << Converter.new({keyword: squery, key_type: 'jan', product_id: asin})
            jj[squery] = {keyword: squery, key_type: 'jan', product_id: asin}
          end
        else
